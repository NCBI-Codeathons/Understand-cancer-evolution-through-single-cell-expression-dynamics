...
def chrName2Int(chr, species):
    ...
    if species == "human":
        ...
        if chr=="X":                #    encode chr X as 20
            return 20           
        elif chr=="Y":              #    encode chr Y as 21
            return 21
        else:
            return int(chr)
...
def infer_cnv(genePosFile, chrFile, expdf, narounds=50, method='genes', species='human'):
        ...
        for line in f:
            ...
            if arr[1]!="MT":
                if (arr[0] not in geneD):
                    geneD[arr[0]]=[chrName2Int(arr[1], species),int(arr[2]),int(arr[3])]
                else:
                    start=geneD[arr[0]][1]
                    end=geneD[arr[0]][2]
    ...
    with open(chrFile, "r") as f:
        for line in f:
            ...
            if (arr[0].isnumeric()) or (arr[0]=="X") or (arr[0]=="Y"):
                chrL[chrName2Int(arr[0], species)]=int(arr[1])
                #print(arr[0]+" "+arr[1])     
